...
let add_settlement_at_point_for_player brd stlmt_type point active_color = (* Yields new board *)
  ...
  let new_inters = replace_at intrs point (Some (active_color, stlmt_type)) in
...
let finish_move s =
  ...
  let res = None in (* if ((victory_points_for_player board player_list color) > cWIN_CONDITION) then Some(color) else None in *)
  (res, (board, player_list, turn, (color, curr_req)))
...
let handle_move s m =
  ...
  | DiscardRequest ->
    ...
    then (None, s) (* TODO: Output issue (None, (board, player_list, turn, ((next_turn color), DiscardRequest))) *)
  ...
  | TradeRequest -> (match m with
    | TradeResponse b -> 
      ...
      if (not b) || turn.pendingtrade = None then (* Trade rejected, try another move *)
        finish_move (board, player_list, new_turn_rcrd, (turn.active, curr_req))
      else (* Conduct trade *) let (clr, c1, c2) = get_some turn.pendingtrade in
    ...
    | Action a -> (match a with
      | RollDice -> let roll_num = Util.random_roll () in
          ...
          if roll_num = cROBBER_ROLL then
            ...
            if (List.length hogs) > 0 then (* Some people have too many resources *)
              ...
              in finish_move (new_board, player_list, rolled_turn, (first_hoarder_color, DiscardRequest)) (* Send discard request *)
            else 
              (* let active_color = turn.active in
              let hex_corners = piece_corners new_robber_loc in
              let (intrs, rds) = str in
              let settlements = intrs in
              let rec adjacent_settlement_color corners = match corners with
                ...
                | h::t -> (match (List.nth settlements h) with
                  | None -> adjacent_settlement_color t
                  | Some (color, stlmt) -> if color <> active_color then Some(color) else adjacent_settlement_color t)
                | [] -> None
              in let adj_color = adjacent_settlement_color hex_corners in *) (* Not sure what this is doing *)
              finish_move (new_board, player_list, rolled_turn, (color, RobberRequest))
          else (* Didn't roll robber, generate resources *)
            let (mp, str, dk, dscrd, rbr) = board in
            let activated_pieces = pieces_for_roll mp roll_num in
            let (intrs, rds) = str in
            let itrsc_list = intrs in
        ...
        let trade_ratio = if List.length settled_ports = 0 then 4 else
          let rec find_resourceful_port_ratio prtlst = (match prtlst with
            | (ln, rtio, prtrsrc)::t -> (match prtrsrc with
              | PortResource(rsrc) -> if rsrc = resource1 then rtio else find_resourceful_port_ratio t
              | Any -> find_resourceful_port_ratio t)
      ...
      | BuyBuild b -> let resources_needed = cost_of_build b in
        ...
        then 
          ...
          then finish_move (board, player_list, turn, (color, ActionRequest)) (* TODO: Should roll dice here *)
          else finish_move (board, player_list, turn, (color, ActionRequest))
          ...
          | BuildCard -> let (mp, str, dk, dscrd, rbr) = board in
            ...
            let revealed_new_deck = wrap_reveal remaining_deck in
            ...
            let new_board = (mp, str, revealed_new_deck, dscrd, rbr) in 
            finish_move (new_board, new_player_list, new_turn_rcrd, (color, curr_req))
          )
      | PlayCard pc -> if turn.cardplayed then finish_move (board, player_list, turn, (color, curr_req)) else (* Only one dev card per turn *)
        ...
        | PlayKnight (rbrmv) -> let (clr, hnd, troph) = get_current_player player_list color in
          ...
          let new_trophies = if is_largest_army && over_threshold then (new_knight_count, r, true) else (new_knight_count, r, false) in
          let new_player_list = update_player_trophies player_list new_trophies color [] in
          (* handle_move_helper (board, new_player_list, new_turn_rcrd, (color, RobberRequest)) (RobberMove(rbrmv)) *)
          (* TODO: Make robber request *)
          finish_move (board, new_player_list, new_turn_rcrd, (color, RobberRequest))
        | PlayRoadBuilding (rd, rdopt) ->
          ...
          let new_board = (mp, (intrs, new_roads), dk, dsc, rbr) in
          finish_move (new_board, player_list, new_turn_rcrd, (color, curr_req))
      ...
      | EndTurn -> 
        ...
        | None -> finish_move (board, player_list, turn, (color, ActionRequest))) (* TODO: Should roll dice *)
      )
    | _ -> failwith "Bad move")
